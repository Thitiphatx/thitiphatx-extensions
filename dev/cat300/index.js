...
const cat300Parser_1 = require("./cat300Parser");
...
class cat300 extends paperback_extensions_common_1.Source {
    constructor() {
        super(...arguments);
        this.cookies = [
    ...
    getMangaShareUrl(mangaId) { return `${BASE_URL}/manga/${mangaId}`; }
    async getMangaDetails(mangaId) {
        ...
        return (0, cat300Parser_1.parseMangaDetails)($, mangaId);
    }
    async getChapters(mangaId) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return (0, cat300Parser_1.parseChapters)($, mangaId);
    }
    async getChapterDetails(mangaId, chapterId) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return (0, cat300Parser_1.parseChapterDetails)($, mangaId, chapterId);
    }
    async filterUpdatedManga(mangaUpdatesFoundCallback, time, ids) {
        ...
        while (updatedManga.loadMore) {
            ...
            const response = await this.requestManager.schedule(request, 1);
            const $ = this.cheerio.load(response.data);
            updatedManga = (0, cat300Parser_1.parseUpdatedManga)($, time, ids);
    ...
    async getHomePageSections(sectionCallback) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        (0, cat300Parser_1.parseHomeSections)($, sectionCallback);
    }
    async getViewMoreItems(homepageSectionId, metadata) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        const manga = (0, cat300Parser_1.parseViewMore)($);
        metadata = !(0, cat300Parser_1.isLastPage)($) ? { page: page + 1 } : {};
    ...
    async getSearchResults(query) {
        ...
        if (query.title) {
            ...
            url = `${BASE_URL}/genres/${query?.includedTags?.map((x) => x.id)[0]}/`;
        }
        const request = createRequestObject({
            url,
            method: 'GET',
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        const manga = (0, cat300Parser_1.parseSearch)($);
        return createPagedResults({
            results: manga,
        });
    }
    async getTags() {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return (0, cat300Parser_1.parseTags)($) || [];
    }
}
exports.cat300 = cat300;

},{"./cat300Parser":57,"paperback-extensions-common":12}],57:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
const parseMangaDetails = ($, mangaId) => {
    const titles = [];
    const title = $('div.wrap > div > div.site-content > div > div.profile-manga.summary-layout-1 > div > div > div > div.post-title > h1').text();
    titles.push(title);
    let image = encodeURI($('div.wrap > div > div.site-content > div > div.profile-manga.summary-layout-1 > div > div > div > div.tab-summary > div.summary_image > a > img').attr('src')) ?? 'https://i.imgur.com/GYUxEX8.png';
    const author = $('div.wrap > div > div.site-content > div > div.profile-manga.summary-layout-1 > div > div > div > div.tab-summary > div.summary_content_wrap > div > div.post-content > div:nth-child(4) > div.summary-content > div > a').text().trim();
    const artist = $('div.wrap > div > div.site-content > div > div.profile-manga.summary-layout-1 > div > div > div > div.tab-summary > div.summary_content_wrap > div > div.post-content > div:nth-child(5) > div.summary-content > div > a').text().trim();
    const rawStatus = $('div.wrap > div > div.site-content > div > div.profile-manga.summary-layout-1 > div > div > div > div.tab-summary > div.summary_content_wrap > div > div.post-status > div.post-content_item > div.summary-content').text().trim();
    let hentai = true;
...
const parseChapters = ($, mangaId) => {
    ...
    for (const chapter of $('li', 'div.wrap > div > div.site-content > div > div.c-page-content.style-1 > div > div > div > div.main-col.col-md-8.col-sm-8 > div > div.c-page > div > div.page-content-listing.single-page > div > ul').toArray()) {
        ...
        const chapterId = $('a', chapter).attr("href").split("/")[5] ?? "";
        if (!chapterId)
            continue;
...
const parseChapterDetails = ($, mangaId, chapterId) => {
    ...
    for (const images of $('img', 'div.wrap > div > div.site-content > div > div > div > div > div > div > div.c-blog-post > div.entry-content > div > div > div > div').toArray()) {
        ...
        if (image && image.startsWith('/'))
            image = 'https:' + image;
        if (image)
            pages.push(encodeURI(image).replace("%0A", ""));
    }
...
const parseUpdatedManga = ($, time, ids) => {
    ...
    for (const manga of $('div.utao.styletwo > div.uta', '#content > div.wrapper > div.postbody > div.bixbox > div.listupd').toArray()) {
        const id = $('div.luf > a.series', manga).attr('href').split('/')[4] ?? '';
        const date = $('div.luf > ul > li:nth-child(1) > span', manga).text() ?? '';
...
const parseHomeSections = ($, sectionCallback) => {
    ...
    for (const row of $('#loop-content > div.page-listing-item > div.row.row-eq-height').toArray()) {
        for (const col of $('div.col-6.col-md-2.badge-pos-1 > div.page-item-detail', row).toArray()) {
            ...
            const id = $(data).attr("href").split("/")[4] ?? "";
            const title = $(data).attr("title") ?? "";
            const image = $('img', data).attr("src") ?? "";
            const latest_num = parseChapterNumber($('div.item-summary > div.list-chapter > div:nth-child(1) > span.chapter.font-meta > a', col).text());
            const subtitle = `Chapter ${latest_num}`;
            if (!id || !title)
                continue;
